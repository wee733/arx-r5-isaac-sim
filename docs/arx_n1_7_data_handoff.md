...
`docs/arx_raw_recorder.py` 不依赖 Isaac Sim，可复制进你的 Isaac 工程。在 action 已完成 IK/控制器处理、但 physics 尚未推进的位置调用：
...
`arx_r5a_vla_collect.launch.py` 的生产 `vla_recorder` 已接入同一份
ARX raw-v1 writer。默认只提交 `SUCCEEDED` episode；`FAILED`、`ABORTED`、
writer 异常和进程中断产生的临时目录都会被丢弃。默认输出根目录是
`~/arx_raw`，可显式指定到数据盘：

```bash
./scripts/run_vla_collect.sh \
  recorder_output_dir:=/data/arx_raw \
  auto_start:=True \
  episode_count:=5
```

采集时 `record_depth` 必须保持 `False`，因为 raw-v1 只包含两路 RGB。
启动日志中的 `raw-v1 recorder writing to ...` 会打印最终输出根目录；每个
成功 episode 结束后，该目录下会原子出现 `episode_XXXXXX`。
